...
function runCommand(input, state, setState) {
  ...
  if (cmd === "systemctl") {
    ...
    if (sub === "status") {
      ...
      return lines([
        ...
        `   Active: ${up ? "active (running)" : "inactive (dead)"} since ${new Date().toLocaleString("it-IT")}`,
  ...
  if (cmd === "journalctl") {
    ...
    if (raw.includes("mysqld")||(raw.includes("-u")&&raw.includes("mysql"))) {
      if (state.mysqlBroken) return lines([
        ...
        "# CAUSA: /var/lib/mysql ha permessi root:root invece di mysql:mysql",
      ]);
    }
    if (raw.includes("sshd")||(raw.includes("-u")&&raw.includes("ssh"))) {
      if (state.bruteForce) return lines([
        ...
        "# ... 4800 tentativi nelle ultime 2 ore da 185.234.12.45",
      ]);
    }
    ...
    if (raw.includes("-xe")||raw.includes("httpd")) {
      if (state.portConflict) return lines([
        ...
        "# CAUSA: un altro processo sta usando la porta 80",
      ]);
      if (state.sslExpired) return lines([
        ...
        "# CAUSA: certificato SSL scaduto — rinnovo necessario",
      ]);
  ...
  if (cmd === "openssl") {
    if (raw.includes("x509") && raw.includes("enddate")) {
      if (state.sslExpired) return lines([
        ...
        "# SCADUTO — oggi è Mar 4 2025. Rinnova il certificato.",
      ]);
      return lines(["notAfter=Mar 04 00:00:00 2026 GMT  ← valido"]);
    }
    if (raw.includes("req") || raw.includes("genrsa")) {
      return ok("Generazione chiave/CSR completata → /etc/pki/tls/certs/server.crt rinnovato.\nOra: systemctl restart httpd");
    }
  ...
  if (cmd === "cp" && raw.includes(".crt")) {
    setState(s => ({ ...s, sslRenewed: true, fixes: { ...s.fixes, sslrenew: true } }));
    return ok("Certificato copiato. Ora: systemctl restart httpd");
  }
  ...
  if (cmd === "df") {
    if (state.inodesFull) return lines([
      ...
      "/dev/mapper/ol-root      50G   18G   32G  36% /  ← spazio libero ma...",
  ...
  if (cmd === "df" && raw.includes("-i")) {
    if (state.inodesFull) return lines([
      ...
      "/dev/mapper/ol-root    3276800 3276800      0  100% /  ← INODE ESAURITI!",
    ]);
  ...
  if (cmd === "touch" || cmd === "mkdir") {
    if (state.inodesFull) return err(`${cmd}: cannot create '${args[0]||"file"}': No space left on device  (inode esauriti)`);
    return ok(`${cmd} completato.`);
  }
  if (cmd === "find" && raw.includes("count")) {
    if (state.inodesFull) return warn("3.276.800 file trovati in /tmp/sessions — sessioni PHP orfane non eliminate");
  ...
  if (raw.includes("find") && raw.includes("/tmp") && (raw.includes("-delete")||raw.includes("-exec rm"))) {
    if (state.inodesFull) {
      ...
      return ok("3.276.612 file eliminati da /tmp/sessions. Inode liberi: 2.826.800");
    }
  }
  ...
  if ((cmd === "ss" || cmd === "netstat") && (raw.includes("-tlpn")||raw.includes("-tulpn")||raw.includes("-lnp"))) {
    if (state.portConflict) return lines([
      ...
      "tcp    LISTEN 0.0.0.0:80         users:((\"nginx\",pid=7701,fd=6))  ← nginx occupa porta 80!",
  ...
  if ((cmd === "ss"||cmd === "netstat") && !raw.includes("-")) {
    if (state.portConflict) return lines(["tcp LISTEN 0.0.0.0:80  nginx  ← conflitto!","tcp LISTEN 0.0.0.0:22  sshd"]);
    return lines(["tcp LISTEN 0.0.0.0:22","tcp LISTEN 0.0.0.0:80","tcp LISTEN 0.0.0.0:3306"]);
  }
  if (cmd === "lsof" && raw.includes(":80")) {
    if (state.portConflict) return lines([
      "COMMAND   PID  USER  FD  TYPE  DEVICE   NAME",
      "nginx    7701  root  6u  IPv4  0t0      TCP *:80 (LISTEN)  ← nginx sul 80",
    ]);
  ...
  if (cmd === "systemctl" && args[0]==="stop" && args[1]==="nginx") {
    if (state.portConflict) {
      ...
      return ok("Stopped nginx.service — porta 80 libera. Ora: systemctl start httpd");
    }
  }
  if (cmd === "kill" && raw.includes("7701")) {
    if (state.portConflict) {
      setState(s => ({ ...s, portConflict: false, fixes: { ...s.fixes, port: true } }));
      return ok("nginx (7701) terminato — porta 80 libera. Ora: systemctl start httpd");
    }
  }
  ...
  if (cmd === "sudo") {
    ...
    return ok(`[sudo] eseguito: ${args.join(" ")}`);
  }
  if (cmd === "visudo") {
    if (state.sudoersBroken) {
      setState(s => ({ ...s, sudoersFixed: true, fixes: { ...s.fixes, sudoers: true } }));
      return ok("visudo: /etc/sudoers: syntax corretta. Permessi ripristinati (0440).\nsudo operativo.");
    }
    return ok("visudo: file aperto. Nessun errore rilevato.");
  }
  if (cmd === "cat" && raw.includes("sudoers")) {
    if (state.sudoersBroken) return lines([
      ...
      "root    ALL=(ALL)   ALL",
      "# line 28 — ERRORE:",
      "%wheel  ALL=(ALL)   NOPASSWD ALl  ← 'ALl' dovrebbe essere 'ALL'",
      "# Inoltre: chmod 777 applicato per errore",
    ]);
    return lines(["root    ALL=(ALL) ALL","%wheel  ALL=(ALL) NOPASSWD: ALL"]);
  }
  if (cmd === "ls" && raw.includes("/etc/sudoers")) {
    if (state.sudoersBroken) return lines(["-rwxrwxrwx 1 root root 755  ← PERICOLOSO! Deve essere -r--r----- (0440)"]);
    return lines(["-r--r----- 1 root root /etc/sudoers"]);
  }
  if (cmd === "chmod" && raw.includes("sudoers")) {
    if (state.sudoersBroken) {
      setState(s => ({ ...s, fixes: { ...s.fixes, sudochmod: true } }));
      return ok("Permessi corretti. Ma risolvi anche il syntax error con visudo.");
    }
  }
  ...
  if (cmd === "cat" && raw.includes("mdstat")) {
    if (state.raidDegraded) return lines([
      ...
      "      104856832 blocks super 1.2 [2/1] [U_]  ← disco sdb1 MANCANTE",
      "",
      "# Array degradato — un disco ha fallito",
    ]);
  ...
  if (cmd === "mdadm") {
    if (raw.includes("--detail")) return state.raidDegraded ? lines([
      ...
      "  Active Devices : 1",
      "Failed Devices : 1  (sdb1 rimosso/fallito)",
  ...
  if (cmd === "smartctl") {
    if (state.raidDegraded && (raw.includes("sdb")||raw.includes("-a"))) return lines([
      ...
      "# sdb è morto — sostituisci il disco fisico e poi mdadm --add",
    ]);
  ...
  if (cmd === "logrotate") {
    if (raw.includes("-d") || raw.includes("--debug")) {
      if (state.logrotBroken) return lines([
        ...
        "error: /etc/logrotate.d/httpd:8 unknown option 'dailys'  ← errore typo",
        "# CAUSA: 'dailys' dovrebbe essere 'daily'",
      ]);
  ...
  if (cmd === "cat" && raw.includes("logrotate.d")) {
    if (state.logrotBroken) return lines([
      ...
      "    missingok",
      "    notifempty",
      "    dailys         ← ERRORE TYPO — deve essere 'daily'",
      "    rotate 7",
      "    compress",
      "}",
    ]);
  ...
  if ((cmd === "vi" || cmd === "nano" || cmd === "sed") && raw.includes("logrotate")) {
    if (state.logrotBroken) {
      ...
      return ok("'dailys' corretto in 'daily'. Ora: logrotate -f /etc/logrotate.conf");
    }
    return ok("File aperto.");
  }

  // ── COMMON ────────────────────────────────────────────────────────────────
  if (cmd === "du") {
    if (state.diskFull)    return lines(["48G\t/var/log","# access_log non ruotato"]);
    if (state.journalFull) return lines(["40G\t/var/log/journal","# journald senza limite"]);
    if (state.lvmFull)     return lines(["98G\t/data/db","# database riempie /data"]);
  ...
  if (cmd === "xfs_growfs"||cmd === "resize2fs") {
    ...
    return ok("Filesystem extended — /data ora a 150G.");
  }
  if (cmd === "mount") {
  ...
  if (raw.includes("umount") && raw.includes("-l")) { setState(s => ({ ...s, nfsHang: false, fixes: { ...s.fixes, nfs: true } })); return ok("Lazy umount completato."); }
  ...
  if (cmd === "cat" && raw.includes("named.conf")) {
    if (state.dnsBroken) return lines(['options {','  forwarders-ip { 8.8.8.8; };  ← ERRORE','};']);
  ...
  if ((cmd==="vi"||cmd==="nano"||cmd==="sed") && raw.includes("named.conf")) {
    ...
    return ok("named.conf corretto. Ora: systemctl start named");
  }
  ...
  if (cmd === "lastb") return state.bruteForce ? lines(["root  185.234.12.45  Mon Mar  4 10:00","# 4800 righe — brute force"]) : lines(["Nessun tentativo fallito."]);
  ...
  if (cmd === "grubby") {
    if (raw.includes("--info")) return lines(["index=0  kernel=vmlinuz-5.15.0-206.153.7.el8uek  ← ATTUALE (panic)","index=1  kernel=vmlinuz-5.15.0-200.130.1.el8uek  ← STABILE"]);
    if (raw.includes("--set-default") && raw.includes("200.130")) {
      setState(s => ({ ...s, kernelBad: false, fixes: { ...s.fixes, kernel: true } }));
      return ok("Default kernel impostato: 5.15.0-200.130.1. Applica con: reboot");
    }
  }
  if (cmd === "rpm" && raw.includes("kernel")) return lines(["kernel-uek-5.15.0-206.153.7  ← recente","kernel-uek-5.15.0-200.130.1  ← precedente"]);
  if (cmd === "date") return state.timeDesync ? lines(["Mon Mar  4 08:00:23 UTC 2024  ← SBAGLIATO"]) : lines([new Date().toUTCString()]);
  if (cmd === "timedatectl") return state.timeDesync
    ? lines(["Local time: Mon 2024-03-04 08:00:23 UTC","NTP active: no  ← PROBLEMA","NTP synchronized: no"])
    : lines(["NTP active: yes","NTP synchronized: yes"]);
  if (cmd === "chronyc") {
    if (raw.includes("sources")) return state.timeDesync ? lines(["^? ntp1.example.com  Reach=0  ← nessun server raggiungibile"]) : lines(["^* pool.ntp.org  +12ms"]);
  ...
  if (cmd === "ausearch") { if (!state.selinuxIssue) return ok("No AVC denials found."); return lines(["avc: denied { read } for httpd","  tcontext=user_home_t  ← SBAGLIATO"]); }
  if (cmd === "ls" && raw.includes("-Z")) { if (state.selinuxIssue) return lines(["user_home_t  index.html  ← SBAGLIATO"]); return lines(["httpd_sys_content_t  index.html"]); }
  if (cmd === "ls" && raw.includes("/var/lib/mysql")) { if (state.mysqlBroken) return lines(["drwxr-xr-x 2 root root mysql  ← deve essere mysql:mysql"]); return lines(["drwxr-xr-x 2 mysql mysql mysql"]); }
  if (cmd === "chcon" && raw.includes("httpd_sys_content_t")) { setState(s => ({ ...s, selinuxIssue: false, fixes: { ...s.fixes, selinux: true } })); return ok("Contesto aggiornato. Esegui: systemctl restart httpd"); }
  if (cmd === "restorecon") { if (state.selinuxIssue) { setState(s => ({ ...s, selinuxIssue: false, fixes: { ...s.fixes, selinux: true } })); return ok("Contesto ripristinato."); } return ok("Nessun contesto da correggere."); }
  if (cmd === "chown" && raw.includes("mysql") && raw.includes("/var/lib/mysql")) { setState(s => ({ ...s, mysqlBroken: false, fixes: { ...s.fixes, mysql: true } })); return ok("Permessi mysql:mysql corretti. Ora: systemctl start mysqld"); }
  if (cmd === "stat" && raw.includes("cron")) { return state.cronBroken ? lines(["Access: (0700/drwx------) Uid: (0/root)  ← deve essere 755"]) : lines(["Access: (0755/drwxr-xr-x)"]); }
  if (cmd === "chmod" && raw.includes("755") && raw.includes("cron")) { setState(s => ({ ...s, fixes: { ...s.fixes, cronfix: true } })); return ok("Permessi corretti. Ora: systemctl restart crond"); }
  if (cmd === "crontab" && raw.includes("-l")) return lines(["0 2 * * * /opt/backup.sh"]);
  if (cmd === "ip") {
    if (raw.includes("route")||raw.split(" ").includes("r")) { if (state.networkDown) return lines(["192.168.1.0/24 dev eth0","# MANCA default gateway"]); return lines(["default via 192.168.1.1 dev eth0"]); }
  ...
  if (cmd === "nmcli") {
    if (raw.includes("modify") && raw.includes("gw4")) { setState(s => ({ ...s, networkDown: false, fixes: { ...s.fixes, network: true } })); return ok("Gateway 192.168.1.1 configurato."); }
    if (raw.includes("con") && raw.includes("up")) { setState(s => ({ ...s, networkDown: false })); return ok("Connessione riattivata."); }
  ...
  if (cmd === "ps" || cmd === "top") {
    const zz = state.zombies ? ["root  8811  8800  Z  0:00 [defunct]","# 50 zombie con PPID 8800"] : [];
  ...
  if (cmd === "kill"||cmd === "killall") {
    if (state.cpuHog && (raw.includes("4512")||raw.includes("stress-ng"))) { setState(s => ({ ...s, cpuHog: null, fixes: { ...s.fixes, cpu: true } })); return ok("Processo terminato. CPU normale."); }
    if (state.swapExhausted && (raw.includes("5511")||raw.includes("java"))) { setState(s => ({ ...s, swapExhausted: false, fixes: { ...s.fixes, swap: true } })); return ok("java terminato. RAM liberata."); }
    if (state.zombies && raw.includes("8800")) { setState(s => ({ ...s, zombies: false, fixes: { ...s.fixes, zombie: true } })); return ok("Padre terminato. 50 zombie rimossi."); }
    if (state.zombies) return warn("Zombie non rispondono ai segnali — killa il padre (PID 8800).");
    return warn("Processo non trovato.");
  }
  if (cmd === "free") { if (state.swapExhausted) return lines(["Mem: 7.7Gi  7.6Gi   12Mi  ← CRITICO","Swap: 2.0Gi  2.0Gi    0B"]); return lines(["Mem: 7.7Gi  2.1Gi  3.8Gi","Swap: 2.0Gi    0B  2.0Gi"]); }
  if (cmd === "vmstat") { if (state.swapExhausted) return lines(["si=980 so=1020  ← swap pesante"]); return lines([" 0  0  0 3800000  0  0"]); }
  if (cmd === "find") {
    if (state.diskFull && raw.includes("/var/log")) return lines(["/var/log/httpd/access_log  48G"]);
    if (state.inodesFull && raw.includes("/tmp")) return warn("3.276.800 file in /tmp/sessions — sessioni PHP orfane\nUsa: find /tmp -type f -delete");
    return lines(["/var/log/httpd/access_log  2.1M"]);
  }
  if (cmd === "truncate" && raw.includes("access_log")) { if (state.diskFull) { setState(s => ({ ...s, diskFull: false, fixes: { ...s.fixes, disk: true } })); return ok("48G liberati."); } return warn("Disco non pieno."); }
  if (cmd === "rm" && raw.includes("access_log")) { if (state.diskFull) { setState(s => ({ ...s, diskFull: false, fixes: { ...s.fixes, disk: true } })); return ok("48G liberati."); } return warn("Non necessario."); }
  if (cmd === "uptime") return lines([` load average: ${state.cpuHog||state.swapExhausted?"3.98, 3.85":"0.12, 0.08"}`]);
  ...
  if (cmd === "cat" && raw.includes("journald"))   return lines(["[Journal]","#SystemMaxUse=  ← nessun limite"]);
  if (cmd === "clear") return [{ text: "__CLEAR__", type: "clear" }];
  if (cmd === "") return [];

  if (cmd === "hint") {
    const H = {
      webdown:    "journalctl -u httpd → OOM → riavvia httpd.",
      ...
      zombie:     "pstree → kill -9 8800 (padre).",
      journal:    "journalctl --disk-usage → --vacuum-size=500M.",
      nfs:        "showmount -e / rpcinfo → umount -l del mount bloccato.",
      lvm:        "pvs/vgs/lvs → lvextend -L +50G → xfs_growfs /data.",
      dnsfail:    "journalctl -u named → cat /etc/named.conf → correggi 'forwarders-ip' → start.",
      sshbrute:   "lastb → journalctl -u sshd → systemctl start fail2ban.",
      kernpanic:  "grubby --info=ALL → --set-default con kernel stabile → reboot.",
      timedesync: "timedatectl → systemctl start chronyd → chronyc sources.",
      ssl:        "openssl x509 -enddate → certbot renew O openssl req → systemctl restart httpd.",
      inode:      "df -i → df -i mostra 100% inode → find /tmp -type f -delete.",
      port:       "journalctl -u httpd → ss -tlpn → nginx occupa :80 → stop nginx → start httpd.",
      sudoers:    "cat /etc/sudoers → ls -la /etc/sudoers → visudo per correggere sintassi.",
      raid:       "cat /proc/mdstat → smartctl -a /dev/sdb → sostituisci disco → mdadm --add.",
      logrotate:  "logrotate -d /etc/logrotate.conf → errore 'dailys' → correggi in 'daily' → logrotate -f.",
      free:       "Esplora: systemctl list-units, df -h, ps aux, free -h.",
    };
  ...
  if (cmd === "help") return lines([
    ...
    "  hint — suggerimento | clear — pulisce lo schermo",
  ]);
...
export default function App({ region = "US", labProgress = { completed: 0, total: 3 } }) {
  ...
  function startScenario(s) {
    ...
    const intro = {
      webdown:    "⚠  ALERT: sito irraggiungibile. Utenti segnalano timeout.",
      diskfull:   "⚠  ALERT: /dev/mapper/ol-root al 100%. Scritture bloccate.",
      selinux:    "⚠  ALERT: httpd risponde ma file danno 403 Forbidden.",
      highcpu:    "⚠  ALERT: load average 3.98. Server irresponsivo.",
      sshfail:    "⚠  ALERT: SSH non accetta connessioni dall'esterno.",
      mysql:      "⚠  ALERT: database down dopo aggiornamento. App ferme.",
      swap:       "⚠  ALERT: OOM killer attivo. RAM esaurita.",
      cron:       "⚠  ALERT: backup notturno non parte da 3 settimane.",
      network:    "⚠  ALERT: server non raggiunge internet.",
      zombie:     "⚠  ALERT: 50 processi in stato Z. Sistema degradato.",
      journal:    "⚠  ALERT: /var/log/journal occupa 40G.",
      nfs:        "⚠  ALERT: mount NFS bloccato. Processi appesi in I/O wait.",
      lvm:        "⚠  ALERT: /data al 100%. Database non scrive.",
      dnsfail:    "⚠  ALERT: DNS down. Nomi host non risolvibili.",
      sshbrute:   "⚠  ALERT: IDS segnala 4800 tentativi SSH in 2 ore.",
      kernpanic:  "⚠  ALERT: kernel panic al boot dopo aggiornamento.",
      timedesync: "⚠  ALERT: certificati TLS rifiutati — orologio 2 ore indietro.",
      ssl:        "⚠  ALERT: HTTPS down — certificato SSL scaduto. Sito irraggiungibile.",
      inode:      "⚠  ALERT: impossibile creare file — 'No space left' ma df mostra 36% usato.",
      port:       "⚠  ALERT: Apache non parte dopo deploy. Porta 80 in conflitto.",
      sudoers:    "⚠  ALERT: sudo è rotto su tutti gli host. Nessun accesso privilegiato.",
      raid:       "⚠  ALERT: array RAID md0 in stato DEGRADED. Disco fallito.",
      logrotate:  "⚠  ALERT: i log non ruotano da settimane. Disco si sta riempiendo.",
      free:       "Shell libera su Oracle Linux 8. Digita 'help' per i comandi.",
    }[s.id] || "Shell pronta.";
    ...
    setTimeout(() => {
      setHistory([
        { text: "Connected to server01.lab.internal [OK]", type: "ok" },
        { text: intro, type: s.id === "free" ? "ok" : "warn" },
        { text: "Digita 'help' per i comandi, 'hint' per un suggerimento.", type: "info" },
      ]);
    }, 600 + Math.random() * 400);
  ...
  function submit() {
    ...
    setTimeout(() => {
      setTermState(st => {
        ...
        if (solved) {
          setHistory(h => [...h, { text: "✅  SCENARIO RISOLTO! Sistema ripristinato correttamente.", type: "ok" }]);
  ...
  return (
    <div style={{ height:"100vh", background:"#060a0f", display:"flex", flexDirection:"column", fontFamily:"'Fira Code','JetBrains Mono',monospace" }}>
      <div style={{ background:"#0a0f14", borderBottom:"1px solid #1a2530", padding:"8px 16px", display:"flex", alignItems:"center", gap:10 }}>
        ...
        <span style={{ color:"#4caf84", fontSize:12, marginLeft:6 }}>root@server01</span>
      ...
      <div style={{ borderTop:"1px solid #1a2530",padding:"10px 18px",display:"flex",alignItems:"center",gap:8,background:"#080d12" }}>
        <span style={{ color:"#4caf84",fontSize:12.5,whiteSpace:"nowrap" }}>[root@server01 ~]$</span>
